...
type SetTypeAsConstant<T extends AnyAction, LiteralType> = {
  [Key in keyof Pick<T, 'type'>]: LiteralType;
};
...
export type ReduxActionType<T extends Fn, LiteralType = unknown> = ReturnType<T> extends AnyAction
  ? {
      [Key in keyof Intersection<ReturnType<T>, LiteralType>]: Intersection<
        ReturnType<T>,
        LiteralType
      >[Key];
    }
...
export type GenericValidationErrors<T> = { [K in keyof T]: any };
